...
describe Checker do
  it 'strips double spaces' do
    ...
    expect(checker.correct_text{ |c| c }).to eq 'This is a double space test.'
  ...
  it 'checks ellipsis' do
    ...
    expect(checker.correct_text{ |c| c }).to eq 'Banane, jabolke,… Ali pa …'
    expect(checker.ellipsis_count).to eq 2
  end

  it 'corrects s placing' do
    checker = Checker.new('S konjem ali z konjem?')
    expect(checker.correct_text{ |c| c }).to eq 'S konjem ali s konjem?'
  ...
  it 'corrects s placing in the beggining of paragraph' do
    ...
    expect(checker.correct_text{ |c| c }).to eq 'S tabo'
    expect(checker.z_count).to eq 1
  end

  it 'corrects z placing' do
    checker = Checker.new('Kaj pa ti s dežnikom?')
    expect(checker.correct_text{ |c| c }).to eq 'Kaj pa ti z dežnikom?'
  ...
  it 'corrects z placing in the beggining of paragraph' do
    ...
    expect(checker.correct_text{ |c| c }).to eq 'Z godbo.'
    expect(checker.s_count).to eq 1
  end

  it 'corrects k placing' do
    checker = Checker.new('Včeraj sem šel k kovaču.')
    expect(checker.correct_text{ |c| c }).to eq 'Včeraj sem šel h kovaču.'
  ...
  it 'corrects k placing in the beggining of paragraph' do
    ...
    expect(checker.correct_text{ |c| c }).to eq 'H kovaču.'
    expect(checker.k_count).to eq 1
  end

  it 'corrects h placing' do
    checker = Checker.new('Pojdi h okulistu!')
    expect(checker.correct_text{ |c| c }).to eq 'Pojdi k okulistu!'
  ...
  it 'corrects h placing in the beggining of paragraph' do
    ...
    expect(checker.correct_text{ |c| c }).to eq 'K okulistu!'
    expect(checker.h_count).to eq 1
  end

  it 'corrects capitals after punctuation' do
    checker = Checker.new('Kaj je to? nič.')
    expect(checker.correct_text{ |c| c }).to eq 'Kaj je to? Nič.'
  ...
  it 'corrects capitals in the beggining of paragraph' do
    ...
    expect(checker.correct_text{ |c| c }).to eq 'Dafak?'
    expect(checker.capitals_count).to eq 1
  ...
  it 'corrects everything at once' do
    ...
    expect(checker.correct_text{ |c| c }).to eq 'This is a double space test, s test,… s konjem ali s konjem? Kaj pa ti z dežnikom? Včeraj sem šel h kovaču. Pojdi k okulistu!'
